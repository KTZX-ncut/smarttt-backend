...
public interface CmAssessmentPlanMapper {
    //获取课程目标数量
    @Select("select count(*) from cm_coursetarget where courseid=#{courseid}")
    int getCourseTargetNum (@Param("courseid") String courseid);

    @Select("select count(*) from cm_assessment_plan_proportion where courseid=#{courseid}")
    int getProportiontNum (@Param("courseid") String courseid);

    @Select("select nProportion,eProportion,courseid from cm_assessment_plan_proportion where courseid=#{courseid}")
    ...
    @Update("update cm_coursetarget set s1=#{s1},s2=#{s2},s3=#{s3},s4=#{s4},s5=#{s5} where id=#{id}")
    void updateItem(CmAssessmentPlanItem cmAssessmentPlanItem);

    @Update("update cm_assessment_plan_proportion set nProportion=#{nProportion},eProportion=#{eProportion} where courseid=#{courseid}")
    void updateProportion(CmAssessmentPlanProportion cmAssessmentPlanProportion);

    @Delete("delete from cm_coursetarget where id=#{id} and courseid=#{courseid}")
    void deleteItem(CmAssessmentPlanItem cmAssessmentPlanItem);

    @Insert("insert into cm_assessment_plan_proportion(nProportion,eProportion,courseid) value(0,0,#{courseid})")
